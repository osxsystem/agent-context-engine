...
impl IndexPipeline {
    pub fn new(repo: String, voyage: Option<EmbedClient>) -> Self {
    ...
    pub async fn run(
        ...
        let (removed_files, new_vectors, incr_stats) = self
            .incremental_run(
                db,
                file_changes,
                progress.as_ref(),
    ...
    async fn streaming_index(
                        ...
                        async move {
                            ...
                            match output {
                                ...
                                ParseOutput::Parsed(pf) => {
                                    ...
                                    let embed_result = match embed_parsed_file(&pf, voyage_ref.as_ref(), cache_ref.clone(), ct_ref.as_ref()).await {
                                        Ok(r) => r,
                                        Err(embed_err) => {
                                            // Cancellation-induced errors come first: if the token
                                            // already fired, or embed_batch reported a cancel, do
                                            // NOT record it as an embedding failure (neither a
                                            // transient skip nor a fatal abort). Leaving the
                                            // err_slot empty lets Stage 3 report a clean
                                            // PipelineAbort::Cancelled instead of EmbeddingFailed.
                                            let inner_err = embed_err.inner();
                                            let cancelled =
                                                crate::embedding::voyage::is_cancel_error(inner_err)
                                                    || ct_ref.as_ref().is_some_and(|ct| ct.is_cancelled());
                                            if cancelled {
                                                return None;
                                            }
        ...
        loop {
            // Race the channel recv against the cancel token. Without this select,
            // a Voyage 429-storm (no item ever arrives) would park `recv()` forever
            // and the Cancel button would do nothing. With it, a user-cancel while
            // parked here resolves the select immediately and breaks the loop.
            let ef = match cancel_token {
                Some(ct) => {
                    tokio::select! {
                        biased;
                        _ = ct.cancelled() => {
                        ...
                        recv = embed_rx.recv() => match recv {
                            Some(ef) => ef,
                            None => break,
                        },
                    }
                }
                None => match embed_rx.recv().await {
                    Some(ef) => ef,
                    None => break,
                },
            };
...
impl EmbedFileError {
    /// Borrow the wrapped error regardless of variant. Used by the streaming
    /// pipeline to test for cancellation (`is_cancel_error`) before deciding
    /// whether to skip (transient) or abort (fatal).
    fn inner(&self) -> &anyhow::Error {
        match self {
            EmbedFileError::Transient(e) | EmbedFileError::Fatal(e) => e,
        }
    }
}
...
async fn embed_parsed_file(
    ...
    match cache {
            ...
            if miss_indices.is_empty() && !raw_hits.is_empty() {
                ...
                if !dim_miss_indices.is_empty() {
                    if let Some(client) = voyage {
                        ...
                        match client.embed(&miss_texts, InputType::Document, cancel).await {
                            Ok(api_results) => {
                ...
                match api_embeddings {
                    Some(api_results) if !api_results.is_empty() => {
                            ...
                            && let Some(client) = voyage
                        {
                            ...
                            match client.embed(&re_texts, InputType::Document, cancel).await {
                                Ok(re_results) => {
                                    let cache_for_put = cache_arc.clone();
        ...
        None => {
            ...
            match voyage {
                Some(client) => match client.embed(&texts, InputType::Document, cancel).await {
...
mod end_to_end_persist {
    ...
    /// REGRESSION (the headline bug): a Voyage 429-storm during a full rebuild
    /// must not hang the pipeline. We point a real EmbedClient at a local
    /// always-429 server (so every embed stalls in the indefinite backoff loop),
    /// kick off `run`, then cancel the token ~150ms later. With the Stage 3
    /// select + cancellable embed_batch, `run` must return promptly with
    /// `PipelineAbort::Cancelled` (not EmbeddingFailed, since the abort is
    /// cancellation-induced). Before the fix this hung forever.
    #[tokio::test]
    async fn full_rebuild_cancels_during_voyage_429_storm() {
        // Local always-429 embeddings endpoint.
        ...
        .unwrap();
        let pipeline = IndexPipeline::new(repo.clone(), Some(client));
        ...
        let result = tokio::time::timeout(
            ...
            pipeline.run(&db, None, true, None, None, None, &[], Some(token)),
        )
        .await
        .expect("run hung past 10s during a 429-storm — cancellation did not unblock it");

        // IndexPipelineStats doesn't implement Debug, so match the result by hand
        // rather than using expect_err (which would format the Ok value).
        let err = match result {
            Ok(_) => panic!("a cancelled 429-storm rebuild must abort, not succeed"),
            Err(e) => e,
        };
        assert!(
            matches!(
            ...
            "expected PipelineAbort::Cancelled (cancellation-induced), got: {err:#}"
        );
    }
}
