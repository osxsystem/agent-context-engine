#!/usr/bin/env bash
# Call-graph endpoint latency oracle — measures GET /api/repos/:repo_id/graph.
#
# WHY: that endpoint recomputes a full-table GROUP BY aggregation over the
# `calls` table on EVERY request. On the real already-indexed Linux kernel repo
# (~4.44M call edges) one request currently takes ~90s. This script is the
# pass/fail oracle for the upcoming fix — it does NOT change any server code, it
# only measures per-request wall-clock latency against the REAL running server
# and emits a machine-readable evidence artifact.
#
# LOCKED GATE: PASS if p50 < 200ms, else FAIL. Printed as `RESULT: PASS|FAIL`.
#
# Usage:
#   scripts/graph_bench.sh <label> [iterations] [repo_id] [port]
# Args / env:
#   <label>       required, e.g. baseline | optimized  (names the artifact)
#   [iterations]  default 20; or env ITERATIONS. Baseline can use a small N
#                 because every request is ~90s on the current no-cache path.
...
#   file    context-engine-rs/bench-results/graph/<label>.json  (evidence)
# Exit code:
...
LABEL="${1:?usage: graph_bench.sh <label> [iterations] [repo_id] [port]}"
...
GRAPH_URL="${URL}/api/repos/${REPO_ID}/graph"
...
OUT_DIR="${CRATE_DIR}/bench-results/graph"
...
echo "[graph_bench] label=${LABEL} repo_id=${REPO_ID} port=${PORT} iterations=${ITERATIONS}"
echo "[graph_bench] target=${GRAPH_URL}"
...
# ── Validate one /graph response: HTTP 200 + JSON with a \"nodes\" array. ──────
# Echoes:  "<http_code> <time_total_seconds>"  on success path (caller checks
# the code). Body validation: must literally contain a "nodes" key. We treat a
# non-200 or a body without "nodes" as a FAILURE, never as a latency sample.
fetch_graph() {  # fetch_graph <body_file> -> prints "<code> <time_total>"
  ...
  code_time="$(curl -sS -m 300 -o "$body" -w '%{http_code} %{time_total}' "$GRAPH_URL" 2>/dev/null || echo '000 0')"
  printf '%s' "$code_time"
}

body_has_nodes() {  # body_has_nodes <body_file> -> rc 0 if valid
  grep -q '"nodes"' "$1" 2>/dev/null
}
...
echo "[graph_bench] warm-up request (not counted) ..."
WARM_CT="$(fetch_graph "$WARM_BODY")"
WARM_CODE="${WARM_CT%% *}"
WARM_SEC="${WARM_CT##* }"
WARM_MS="$(awk -v s="$WARM_SEC" 'BEGIN{printf "%.1f", s*1000}')"
if [ "$WARM_CODE" != "200" ] || ! body_has_nodes "$WARM_BODY"; then
...
echo "[graph_bench] warm-up: http=${WARM_CODE} latency=${WARM_MS}ms (this is the per-request cost on the no-cache path)"

# Node/edge counts from the warm-up body (cheap structural count: each node has a
# distinct \"id\" field, each edge a \"source\" field — count occurrences). These
# describe the payload shape; identical across requests for a static index.
NODES_COUNT="$(grep -o '"id"' "$WARM_BODY" | wc -l | tr -d ' ')"
EDGES_COUNT="$(grep -o '"source"' "$WARM_BODY" | wc -l | tr -d ' ')"
TRUNCATED="$(grep -o '"truncated":[a-z]*' "$WARM_BODY" | head -1 | sed 's/.*://')"
...
for i in $(seq 1 "$ITERATIONS"); do
  CT="$(fetch_graph "$TMP_BODY")"
  CODE="${CT%% *}"
  SEC="${CT##* }"
  CODE_COUNTS["$CODE"]=$(( ${CODE_COUNTS["$CODE"]:-0} + 1 ))
  if [ "$CODE" = "200" ] && body_has_nodes "$TMP_BODY"; then
    MS="$(awk -v s="$SEC" 'BEGIN{printf "%.1f", s*1000}')"
    SAMPLES_MS+=("$MS")
    echo "[graph_bench] iter ${i}/${ITERATIONS}: http=${CODE} ${MS}ms"
  else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    echo "[graph_bench] iter ${i}/${ITERATIONS}: FAILURE http=${CODE} (body invalid?) — not counted in latency stats" >&2
...
echo " payload      : nodes=${NODES_COUNT} edges=${EDGES_COUNT} truncated=${TRUNCATED:-?}"
...
EOF
echo "[graph_bench] artifact -> ${OUT_JSON}"
