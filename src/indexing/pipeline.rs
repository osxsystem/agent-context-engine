        ...
        if file_changes.is_empty() {
            ...
            if !resolved {
                ...
                if raw_edge_total == 0 && !stored_meta.is_empty() {
                    ...
                    self.resolve_edges_phase2(db, cancel_token.as_ref())
                        .await
                            ...
                            match output {
                                ...
                                ParseOutput::Parsed(pf) => {
                                    ...
                                    let embed_result = match embed_parsed_file(&pf, voyage_ref.as_ref(), cache_ref.clone(), ct_ref.as_ref()).await {
                                        Ok(r) => r,
                                        Err(e) => {
                                            // If the error is cancellation-induced (the token
                                            // already fired, or embed_batch reported a cancel),
                                            // do NOT record it as an embedding failure: leaving
                                            // the err_slot empty lets Stage 3 report a clean
                                            // PipelineAbort::Cancelled instead of EmbeddingFailed.
                                            let cancelled = crate::embedding::voyage::is_cancel_error(&e)
        ...
        None => {
            ...
            match voyage {
                Some(client) => match client.embed(&texts, InputType::Document, cancel).await {
                    ...
                    Err(e) => Err(e.context(format!("embed failed for {}", pf.path))),
                },
